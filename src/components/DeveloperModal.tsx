    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
        ...
        <motion.div
          ...
          className="relative w-full max-w-2xl bg-gradient-to-b from-slate-900 via-slate-950 to-slate-900 border border-cyan-500/30 rounded-3xl p-5 sm:p-8 shadow-2xl shadow-cyan-950/60 overflow-hidden z-10"
        >
          ...
          {/* Header Row: Category Badges & Integrated Close Button (no overlap on mobile) */}
          <div className="flex items-start justify-between gap-3 mb-5 sm:mb-6 relative z-10">
            <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
              <div className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/30 text-cyan-300 text-[11px] sm:text-xs font-bold tracking-wide">
                <Sparkles className="h-3.5 w-3.5 text-cyan-400 shrink-0" />
                <span>Desarrollador & Creador</span>
              </div>
              <div className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-[11px] sm:text-xs font-semibold">
                <Leaf className="h-3.5 w-3.5 text-emerald-400 shrink-0" />
                <span>Ingeniería Ambiental + IA</span>
              </div>
            </div>

            {/* Close Button - positioned in header flex flow */}
            <button
              id="btn-close-developer-modal"
              type="button"
              onClick={onClose}
              className="p-1.5 sm:p-2 rounded-xl sm:rounded-2xl bg-white/5 hover:bg-white/15 text-slate-400 hover:text-white transition-all active:scale-95 cursor-pointer border border-white/10 shrink-0"
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 sm:gap-8">
            ...
            <div className="flex flex-col items-center gap-3 shrink-0">
              <div className="relative group">
                ...
                <div className="relative w-32 h-32 sm:w-44 sm:h-44 rounded-full p-1 bg-slate-900 border-2 border-cyan-400/60 shadow-xl overflow-hidden">
                  <img
            ...
            {/* Information Column */}
            <div className="flex-1 text-center sm:text-left space-y-3 sm:space-y-3.5">
              <div>
              ...
              {/* Exact Description */}
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed bg-white/[0.03] p-3.5 sm:p-4 rounded-2xl border border-white/5 font-normal text-left sm:text-left">
              ...
              <div className="pt-1 sm:pt-2 flex flex-col sm:flex-row items-center gap-3">
                <a
                  id="btn-whatsapp-contact"
                  href={whatsappUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2.5 px-5 sm:px-6 py-2.5 sm:py-3 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-bold text-xs sm:text-sm shadow-lg shadow-emerald-500/25 border border-emerald-300/40 transition-all active:scale-95 cursor-pointer group"
                >
                  {/* WhatsApp SVG Icon */}
                  <svg
                    className="w-4 h-4 sm:w-5 sm:h-5 fill-current text-white transition-transform group-hover:scale-110 shrink-0"
                    viewBox="0 0 24 24"
                  >
                  ...
                  <ExternalLink className="h-3.5 w-3.5 text-emerald-200 shrink-0" />
                </a>
              </div>
            </div>
          </div>
