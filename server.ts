...
// Helper to generate the standardized technical report complying strictly with the 12-section official template
function generateStandardizedTechnicalReport(payload: {
  mode?: 'single' | 'all';
  summary?: any;
  metadata?: any;
  samples?: any[];
...
## 1. Resumen ejecutivo
El presente informe técnico expone la evaluación hidroquímica, física y microbiológica de la calidad ambiental en el cuerpo de agua **${waterBody}** (${location}), correspondiente a **${isMulti ? `${totalSamplesCount} puntos de monitoreo` : `el punto de monitoreo ${sampleCode}` }**.
...
${samples
  ...
  .map((s: any, sIdx: number) => {
    ...
    const transgressions = s.summary.transgressionCount;
